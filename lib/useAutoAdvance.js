...
export function useAutoAdvance(data, now) {
  ...
  useEffect(() => {
    ...
    const everyoneAnswered = data.playersCount > 0 && data.answeredCount >= data.playersCount;
    if ((remaining <= 0 || everyoneAnswered) && revealedForRef.current !== data.state.questionIndex) {
      revealedForRef.current = data.state.questionIndex;
      callHostAction('reveal');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [phase, now, data?.state?.startedAt, data?.state?.questionIndex, data?.playersCount, data?.answeredCount]);
