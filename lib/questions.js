...
export const TIME_LIMIT_LONG_MS = 30000;
export const LONG_TEXT_THRESHOLD = 220; // znaků (otázka + všechny odpovědi dohromady)
...
// Delší otázky/odpovědi dostanou víc času na přečtení (20s místo 15s).
export function timeLimitForIndex(index) {
  const item = QUESTIONS[index];
  if (!item) return TIME_LIMIT_MS;
  const length = item.q.length + item.choices.join('').length;
  return length >= LONG_TEXT_THRESHOLD ? TIME_LIMIT_LONG_MS : TIME_LIMIT_MS;
}
